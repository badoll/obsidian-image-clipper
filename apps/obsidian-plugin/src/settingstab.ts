...
export default class SettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        containerEl.createEl("h3", { text: "Authenticated downloads" })

        new Setting(containerEl)
            .setName("Enable authenticated downloads")
            .setDesc("Attach private headers only for exact domains configured in the local auth file.")
            .addToggle((toggle) =>
                toggle
                    .setValue(this.plugin.settings.authDownloadsEnabled)
                    .onChange(async (value) => {
                        this.plugin.settings.authDownloadsEnabled = value
                        await this.plugin.saveSettings()
                    })
            )

        new Setting(containerEl)
            .setName("Auth config file")
            .setDesc("Private auth file path. Leave as default to use the centralized config.json written by the browser extension and native host.")
            .addText((text) =>
                text
                    .setPlaceholder(DEFAULT_AUTH_CONFIG_PATH)
                    .setValue(this.plugin.settings.authConfigPath)
                    .onChange(async (value) => {
                        this.plugin.settings.authConfigPath = value || DEFAULT_AUTH_CONFIG_PATH
                        await this.plugin.saveSettings()
                    })
            )
        ...
        new Setting(containerEl)
            ...
            .setDesc("Minutes after which diagnostics will report that the auth file may be stale.")
            .addText((text) =>
                text
                    .setValue(String(this.plugin.settings.authConfigStaleMinutes))
                    .onChange(async (value: string) => {
                        ...
                        if (
                            ...
                            displayError("The value should be a positive integer number between 5 and 10080!")
                            return
                        }
                        this.plugin.settings.authConfigStaleMinutes = numberValue
                        await this.plugin.saveSettings()
                    })
            )

        new Setting(containerEl)
            .setName("Diagnostics URL")
            .setDesc("Optional fallback URL for the diagnostics command when no URL is selected in the editor.")
            .addText((text) =>
                text
                    .setPlaceholder("https://protected.example.com/asset/...")
                    .setValue(this.plugin.settings.diagnosticsLastUrl)
                    .onChange(async (value) => {
                        this.plugin.settings.diagnosticsLastUrl = value
                        await this.plugin.saveSettings()
                    })
            )
