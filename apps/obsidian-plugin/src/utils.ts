...
export async function downloadImageDetailed(url: string, extraHeaders: Record<string, string> = {}): Promise<DownloadImageResult> {
  ...
  try {
    logError(maskSecrets({ url, headers }), true);
    const res = await requestUrl({ url: url, method: "GET", headers })
    const classification = classifyDownloadResponse({
      status: res.status,
      headers: res.headers,
      arrayBuffer: res.arrayBuffer
    });

    logError(maskSecrets({
      url,
      status: classification.status,
      contentType: classification.contentType,
      loginPageSuspected: classification.loginPageSuspected,
...
export async function downloadImage(url: string, extraHeaders: Record<string, string> = {}): Promise<ArrayBuffer> {
  const result = await downloadImageDetailed(url, extraHeaders);
  if (!result.ok) {
    const authHint = extraHeaders.Cookie ? " Authentication may be missing, expired, or redirected to a login page." : "";
    logError("Cannot download the file: " + (result.error ?? "unknown error") + authHint, false);
    return null;
  }
...
export async function getFileExt(content: ArrayBuffer, link: string) {
  ...
  const fileExtByBuffer = (await fileTypeFromBuffer(content))?.ext;

  // if XML, probably it is SVG
  if (fileExtByBuffer == "xml" || !fileExtByBuffer) {
    const buffer = Buffer.from(content);
    if (isSvg(buffer.toString("utf8"))) return "svg";
  }
