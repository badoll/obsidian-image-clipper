...
type PluginWithAttachmentFolders = Plugin & {
  ensureFolderExists(folderPath: string): Promise<void>;
};

export function imageTagProcessor(app: PluginWithAttachmentFolders,
  noteFile: TFile,
  settings: ISettings,
  defaultdir: boolean
) {
    ...
    try {
      ...
      if (protocol == "data:") {
        ...
        if (protocol == "file:") {
          ...
          let trycount = 0;
          const authHeaders = await getAuthenticatedHeaders(link, settings);
          while (trycount < settings.tryCount) {
            fileData = await downloadImage(link, authHeaders);
