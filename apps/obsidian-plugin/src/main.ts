...
export default class LocalImagesPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      id: "diagnose-authenticated-download",
      name: "Diagnose authenticated image URL",
      editorCallback: this.diagnoseAuthenticatedDownload,
    })

    this.addCommand({
      id: "import-local-images-plus-settings",
      name: "Import Local Images Plus settings",
      callback: this.importLegacyLocalImagesPlusSettings,
    })
  ...
  private diagnoseAuthenticatedDownload = async (editor: Editor = undefined) => {
    const selected = trimAny((editor?.getSelection() ?? ""), [" ", "\n", "\r", "\t", "(", ")", "[", "]"])
    const url = selected || this.settings.diagnosticsLastUrl

    if (!url || !isUrl(url)) {
      showBalloon("Select an authenticated image URL first, or set a Diagnostics URL in plugin settings.", this.settings.showNotifications)
      return
    }

    this.settings.diagnosticsLastUrl = url
    await this.saveSettings()

    const diagnostics = await diagnoseAuthenticatedUrl(url, this.settings)
    const message = formatDiagnostics(diagnostics)
    logError(message, false)
    showBalloon(message, true, 15000)
  }




  private importLegacyLocalImagesPlusSettings = async () => {
    const configDir = (this.app.vault as any).configDir || ".obsidian"
    const legacyPath = pathJoin([configDir, "plugins", "obsidian-local-images-plus", "data.json"])

    try {
      if (!await this.app.vault.adapter.exists(legacyPath)) {
        showBalloon("Local Images Plus settings were not found at " + legacyPath, this.settings.showNotifications)
        return
      }
      ...
      const protectedSettings = {
        authDownloadsEnabled: this.settings.authDownloadsEnabled,
        authConfigPath: this.settings.authConfigPath,
        authConfigStaleMinutes: this.settings.authConfigStaleMinutes,
        diagnosticsLastUrl: this.settings.diagnosticsLastUrl
      }
      const nextSettings = Object.assign({}, this.settings)

      Object.keys(DEFAULT_SETTINGS).forEach((key) => {
        if (
          key in legacySettings &&
          !key.startsWith("auth") &&
          key !== "diagnosticsLastUrl"
        ) {
          (nextSettings as any)[key] = legacySettings[key]
        }
      })

      this.settings = Object.assign(nextSettings, protectedSettings) as ISettings
      await this.saveSettings()
      showBalloon("Imported Local Images Plus settings. Auth secrets were not imported.", this.settings.showNotifications)
    } catch (error) {
  ...
  private removeOrphans = (type: string = undefined, filesToRemove: Array<TFile> = undefined, noteFile: TFile = undefined) => async () => {
      ...
      if (type == "plugin") {
        ...
        let allAttachmentsLinks: string[] = []
        if (this.settings.saveAttE != "nextToNoteS" ||
        ...
        if (this.ExemplaryOfMD(noteFile.path)) {
          ...
          if (allAttachments) {
            for (const attach of allAttachments) {
              if (attach instanceof TFile && !allAttachmentsLinks.includes(attach.name)) {
